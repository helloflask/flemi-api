...
class PostTestCase(Base):
    ...
    def test_get_post(self):
        ...
        self.assertEqual(resp.json["title"], Post.query.get(1).title)
    ...
    def test_get_posts(self):
        ...
        self.assertEqual(resp.json["posts"][0]["title"], Post.query.get(9).title)

    def test_create_post(self):
        resp = self.client.post(
            "/post", headers=self.headers, json={"title": "lorem", "content": "ipsum"}
        )
        self.assertEqual(resp.status_code, 201)
        resp = self.client.post(
            "/post",
            headers=self.headers,
            json={"title": "post2", "content": "post2", "column_ids": [1, 2]},
        )
    ...
    def test_update_post(self):
        ...
        resp = self.client.put(
            f"/post/{p.id}", headers=self.headers, json={"title": "new"}
        )
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(p.title, "new")
        resp = self.client.put(
            f"/post/{p.id}", headers=self.headers, json={"column_ids": [3]}
        )
        self.assertEqual(resp.status_code, 200)
